<?php
  if(isset($_GET['university_id'])){
    ...
    if($_SESSION['Role']!="Administrator"){
      $check_has_unique_center_code = $conn->query("SELECT Center_Suffix FROM Universities WHERE ID = ".$id." AND Has_Unique_Center = 1");
    ...
    if($_SESSION['Role']=='University Head' || $_SESSION['Role']=='Administrator'){
      $university_query = " AND University_ID = ".$id;
      // $table = 'Alloted_Center_To_Counsellor';
      $table = 'Users';

    }elseif($_SESSION['Role']=='Counsellor'){
      $university_query = " AND University_ID = ".$id." AND Counsellor_ID =".$_SESSION['ID'];
      $table = 'Alloted_Center_To_Counsellor';
    }elseif($_SESSION['Role']=='Sub-Counsellor'){
      $university_query = " AND University_ID = ".$id." AND Sub_Counsellor_ID =".$_SESSION['ID'];
      $table = 'Alloted_Center_To_SubCounsellor';
    }elseif($_SESSION['Role']=='Center'){
      $university_query = " AND University_ID = ".$id." AND Code = ".$_SESSION['ID'];
      $table = 'Alloted_Center_To_Counsellor';
    ...
    $alloted_centers = $conn->query("SELECT ID,Name,Code FROM $table WHERE ID IS NOT NULL $university_query GROUP BY Code");
    while($alloted_center = $alloted_centers->fetch_assoc()){
      $center_ids[] = $alloted_center['ID'];
    }
