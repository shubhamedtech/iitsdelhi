...
if (isset($_GET['session_id']) && isset($_GET['university_id'])) {
  ...
  // echo "SELECT CT_Status, LE_Status FROM Admission_Sessions WHERE ID = $session_id";
  $status = $conn->query("SELECT CT_Status, LE_Status FROM Admission_Sessions WHERE ID = $session_id");
  $status = mysqli_fetch_assoc($status);

  $option = "";
   //echo "SELECT ID, Name FROM Admission_Types WHERE University_ID = $university_id";die;
 ...
 /*    if (strcasecmp($admission_type['Name'], 'DISTANCE') == 0 || strcasecmp($admission_type['Name'], 'fresh') == 0) {
      $option .= '<option value="' . $admission_type['ID'] . '">' . $admission_type['Name'] . '</option>';
    }
    */
  }
