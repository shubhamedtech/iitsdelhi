...
// echo "SELECT ID FROM Universities WHERE Is_Vocational = 1 AND ID = $university_id"; die;
    ...
    if($is_vocational->num_rows>0){
      // ECHO "SELECT Center_Sub_Courses.Course_ID FROM Center_Sub_Courses LEFT JOIN Sub_Courses ON Center_Sub_Courses.Sub_Course_ID = Sub_Courses.ID WHERE `User_ID` = $user_id AND Center_Sub_Courses.University_ID = $university_id $status_query $query GROUP BY Center_Sub_Courses.Course_ID";DIE;
