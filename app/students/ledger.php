    ...
    <div class="col-md-12 d-flex justify-content-end">
      <div>
        ...
        <?php if (in_array($_SESSION['Role'], ['Administrator', 'Accountant'])) { ?>
          <button class="btn btn-primary" disabled onclick="add('offline-payments', 'lg')">Pay Offline</button>
        <?php } ?>
      </div>
    </div>
  </div>
          ...
          <div class="table-responsive">
            <table class="table table-hover table-borderless">
              ...
              <tbody>
                ...
                $balance = 0;
                $credit = 0;
                $ledgers = $conn->query("SELECT Student_Ledgers.*, Payments.Amount as received, Payments.Gateway_ID as art_id FROM Student_Ledgers LEFT JOIN Payments ON Payments.Transaction_ID = Student_Ledgers.Transaction_ID WHERE Student_Ledgers.Student_ID = $id AND Duration <= '" . $student['Duration'] . "' AND Student_Ledgers.Status = 1 ORDER BY Student_Ledgers.Type, Student_Ledgers.Created_At");
                while ($ledger = $ledgers->fetch_assoc()) {
                  ...
                  $credited = $ledger['Type'] == 2 ? $ledger['received'] : 0;
                ...
                ?>
                  <tr>
                    ...
                    <td><?= $ledger['Source'] ?></td>
                    <td><a href="/print/receipt/index.php?id=<?= $ledger['ID'] ?>" target="_blank"><u><?= $ledger['art_id'] ?></u></a></td>
                    <td class="text-right">
                      <?php
                      $debit = ($ledger['Type'] == 1) ? $ledger['Fee'] : $credited;
                      echo "&#8377; " . number_format($debit, 2);
                      ?>
                    </td>
                    <td class="text-right"><?php echo "&#8377; " . number_format($credited, 2); ?></td>
                    <td class="text-right"><?php
                                            $balance = $ledger['Fee'];
                                            $debit = ($ledger['Type'] == 1) ? number_format($balance, 2) : number_format($credited, 2);
                                            echo "&#8377; " . $debit;
                                            ?>
                    </td>
                  </tr>
