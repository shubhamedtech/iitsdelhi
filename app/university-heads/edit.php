...
if (isset($_GET['id'])) {
  ...
  $id = intval($_GET['id']);
  $user = $conn->query("SELECT Name, Code,Photo, Email, Mobile FROM Users WHERE ID = $id");
...
<div class="modal-header clearfix text-left">
  ...
  <h5 class="fs-4 text-black fw-bold">Edit <span class="semi-bold">University Manager</span></h5>
</div>
<!-- <form role="form" id="form-edit-university-heads" action="/app/university-heads/update" method="POST" enctype="multipart/form-data">
...
<form class="card-body" role="form" id="form-edit-university-heads" action="/app/university-heads/update" method="POST" enctype="multipart/form-data">

  <div class="row mb-4">
    <label class="col-sm-3 col-form-label" for="multicol-full-name">Name <span class="text-danger">*</span></label>
    <div class="col-sm-9">
      <input type="text" id="name" name="name" class="form-control" placeholder="ex: Jhon Doe" value="<?= $user['Name'] ?>" required>
    </div>
  </div>
  <div class="row mb-4">
    <label class="col-sm-3 col-form-label" for="multicol-full-name">Employee ID <span class="text-danger">*</span></label>
    <div class="col-sm-9">
      <input type="text" id="code" name="code" class="form-control" placeholder="ex: EM0001" value="<?= $user['Code'] ?>" required>
    </div>
  </div>
  <div class="row mb-4">
    <label class="col-sm-3 col-form-label" for="multicol-full-name">Email <span class="text-danger">*</span></label>
    <div class="col-sm-9">
      <input type="email" id="email" name="email" class="form-control"  placeholder="ex: user@example.com" value="<?= $user['Email'] ?>" required>
    </div>
  </div>
  <div class="row mb-4">
    <label class="col-sm-3 col-form-label" for="multicol-full-name">Contact <span class="text-danger">*</span></label>
    <div class="col-sm-9">
      <input type="tel" id="contact" name="contact" class="form-control"  placeholder="ex: 9998777655" minlength="10" maxlength="10" onkeypress="return isNumberKey(event)" value="<?= $user['Mobile'] ?>" required>
    </div>
  </div>
  <div class="row mb-4">
