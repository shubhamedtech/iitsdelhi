    ...
    if ($check->num_rows > 0) {
      ...
      if ($user_details['Status'] == 1) {
        ...
        if ($_SESSION['Role'] == 'Center' || $_SESSION['Role'] == 'Sub-Center') {
          ...
          while ($dt = $universities->fetch_assoc()) {
            $all_universities[] = $dt['University_ID'];
            $all_universities_name[] = $dt['Name'];
            $all_universities_logo[] = $dt['Logo'];
            if ($counter == 1) {
              $_SESSION['university_id'] = $dt['University_ID'];
              $_SESSION['university_name'] = $dt['Name'];
              $_SESSION['unique_center'] = $dt['Has_Unique_Center'];
          ...
          $_SESSION['university_name'] = array_filter(array_unique($all_universities_name));
          $_SESSION['University_ID'] = array_filter(array_unique($all_universities));
          $_SESSION['Alloted_Universities'] = array_filter(array_unique($all_universities));
          $_SESSION['university_id'] =array_filter(array_unique($all_universities));
          $_SESSION['university_logo'] =array_filter(array_unique($all_universities_logo));
          $universities=implode(',',$_SESSION['Alloted_Universities']);
          $_SESSION['university_ids'] = $universities;
        ...
        $query = $_SESSION['Role'] != "Administrator" ? " AND University_ID IN ($universities)" : '';
        ...
        if ($_SESSION['Role'] === 'Administrator' || $_SESSION['Role'] == 'Operations' || $_SESSION['Role'] == 'Accountant') {
          ...
          // $counsellors = $conn->query("SELECT `User_ID` FROM University_User WHERE User_ID = '" . $_SESSION['ID'] . "' AND University_ID =IN ($universities)" );//. $_SESSION['university_id'])
          // while ($counsellor = $counsellors->fetch_assoc()) {
          //   $counsellor_list[] = $counsellor['User_ID'];
          // }

          $get_all_center = $conn->query("SELECT DISTINCT(Code) as Code FROM Alloted_Center_To_Counsellor WHERE University_ID IN ($universities)");
        ...
        if ($_SESSION['Role'] != 'Administrator') {
          $universities =implode(',',$_SESSION['university_id']);
          $gateway = $conn->query("SELECT * FROM Payment_Gateways WHERE University_ID IN($universities)");
        ...
        if ($_SESSION['Role'] === 'Center'){
          echo json_encode(['status' => 200, 'message' => 'Welcome ' . $user_details['Name'], 'url' => '/admission/applications']);
        }else {
          echo json_encode(['status' => 200, 'message' => 'Welcome ' . $user_details['Name'], 'url' => '/admission/applications']);
