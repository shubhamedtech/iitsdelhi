  ...
  if(isset($_POST['name']) && isset($_POST['email']) && isset($_POST['user_type'])){
    ...
    if($user_type == 1){
      if($_SESSION['Role']=='Administrator'){
        $check_has_unique_center_code = $conn->query("SELECT Center_Suffix FROM Universities WHERE ID IN ($university_id) AND Has_Unique_Center = 1");
...
//     ECHO "INSERT INTO `Users`(`Name`, `Short_Name`, `Contact_Name`, `Code`, `Email`, `Mobile`, `Alternate_Mobile`, `Address`, `Pincode`, `City`, `District`, `State`, `Password`, `Photo`, `Role`, `Designation`, `Is_Unique`, `Created_By`, `B2B_Partner`,`University_ID`) VALUES ('$name', '$short_name', '$contact_person_name', '$code', '$email', '$contact', '$alternate_contact', '$address', '$pincode', '$city', '$district', '$state', AES_ENCRYPT('12345','60ZpqkOnqn0UQQ2MYTlJ'), '$filename', 'Center', 'Center', $is_unique, ".$_SESSION['ID'].", '$user_type','$university_id')"; DIE;
// echo "<pre>"; print_r($_POST); DIE;
    $add = $conn->query("INSERT INTO `Users`(`Name`, `Short_Name`, `Contact_Name`, `Code`, `Email`, `Mobile`, `Alternate_Mobile`, `Address`, `Pincode`, `City`, `District`, `State`, `Password`, `Photo`, `Role`, `Designation`, `Is_Unique`, `Created_By`, `B2B_Partner`,`University_ID`) VALUES ('$name', '$short_name', '$contact_person_name', '$code', '$email', '$contact', '$alternate_contact', '$address', '$pincode', '$city', '$district', '$state', AES_ENCRYPT('12345','60ZpqkOnqn0UQQ2MYTlJ'), '$filename', 'Center', 'Center', $is_unique, ".$_SESSION['ID'].", '$user_type','$university_id')");
    if($add){
      $last_id = $conn->insert_id;
      $checks = $conn->query("SELECT ID FROM Alloted_Center_To_Counsellor WHERE Code = $last_id AND University_ID IN($university_id)");
      foreach ($_POST['university_id'] as $uni_id) {
        if ($checks->num_rows > 0) {
          $update_allot_counsellor = $conn->query("UPDATE Alloted_Center_To_Counsellor SET Counsellor_ID= $university_head, University_ID = $uni_id WHERE Code = $last_id");
        } else {
          $update_allot_counsellor = $conn->query("INSERT INTO Alloted_Center_To_Counsellor (Counsellor_ID,`Code`,`University_ID`) VALUES ($university_head,$last_id, $uni_id)");
        }
      }
