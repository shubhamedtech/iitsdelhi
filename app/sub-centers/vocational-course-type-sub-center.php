  ...
  if ($university_id == 48) {
        ...
        if (count(json_decode($sub_course['durections'])) > 0) {
          foreach (json_decode($sub_course['durections']) as $indx => $drf) {
            ...
            <div class="col-md-2">
              ...
              $defaultValue = isset($fees[$subCourseID]) && isset($fees[$subCourseID][$indx]) ? $fees[$subCourseID][$indx] : '';
              ?>
              ...
              <input type="hidden" name="subcourse_id[<?= $subCourseID ?>]" class="subcourse_id" value="<?= $subCourseID ?>" />
            </div>
            <?php
          }
        } else { ?>
    ...
    foreach ($subCourseData as $sub_course) {
      ...
      <div class="row pb-2">
        ...
        <div class="col-md-3">
          <input type="hidden" id="course_type" name="course_type[]" value="<?= $type_ids ?>">
          <input type="hidden" id="subcourse_id" name="subcourse_id" value="<?= $sub_course['ID'] ?>]">
          <input type="number" min="0" step="500" placeholder="Fee" name="fee[<?= $sub_course['ID'] ?>]"
            value="<?php echo array_key_exists($sub_course['ID'], $fees) ? $fees[$sub_course['ID']] : '' ?>"
            class="form-control" />
        </div>
